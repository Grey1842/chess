...
class GameState:
    def __init__(self):
        """
        Board is an 8x8 array, each element in the array has 2 characters.
        ...
        self.stalemate_reason = ""  # Track the reason for stalemate
        self.in_check = False
        self.pins = []
        self.checks = []
        ...
        # For tracking the 50-move rule and threefold repetition
        self.halfmove_clock = 0  # Incremented after each move, reset after capture or pawn move
        self.position_history = {}  # Dictionary to store position occurrences
        self.current_position = ""  # String representation of current position
        self.updateCurrentPosition()

    def updateCurrentPosition(self):
        """
        Creates a string representation of the current board position for threefold repetition detection.
        Includes board state, castling rights, en passant possibilities, and whose turn it is.
        """
        position = ""
        # Add board state
        for row in range(8):
        ...
        # Add castling rights
        position += f"{int(self.current_castling_rights.wks)}{int(self.current_castling_rights.wqs)}"
        position += f"{int(self.current_castling_rights.bks)}{int(self.current_castling_rights.bqs)}"
        # Add en passant possibility
        position += str(self.enpassant_possible)
        # Add whose turn it is
        position += "w" if self.white_to_move else "b"
        ...
        if self.position_history[position] >= 3:
            self.stalemate = True
            self.stalemate_reason = "Threefold repetition"
    ...
    def makeMove(self, move):
        ...
        if move.piece_captured != "--" or move.piece_moved[1] == "p":
            # Reset clock on capture or pawn move
            self.halfmove_clock = 0
        else:
            # Increment clock otherwise
            self.halfmove_clock += 1
            
        # Check for 50-move rule (100 halfmoves)
        if self.halfmove_clock >= 100:
            self.stalemate = True
            self.stalemate_reason = "50-move rule"
            
        # Update current position for threefold repetition check
        self.updateCurrentPosition()
    ...
    def undoMove(self):
        ...
        if len(self.move_log) != 0:  # make sure that there is a move to undo
            ...
            if move.piece_captured != "--" or move.piece_moved[1] == "p":
                # Adjust halfmove clock based on the move type
                if len(self.move_log) > 0:
                    prev_move = self.move_log[-1]
                    if prev_move.piece_captured != "--" or prev_move.piece_moved[1] == "p":
                        self.halfmove_clock = 0
                    else:
    ...
    def getValidMoves(self):
        ...
        if len(moves) == 0:
            if self.inCheck():
                ...
                self.stalemate = True
                self.stalemate_reason = "No valid moves"
        else:
            self.checkmate = False
            self.stalemate = False
...
class Move:
    ...
    def __init__(self, start_square, end_square, board, is_enpassant_move=False, is_castle_move=False):
        ...
        # For pawn promotion choice (default is Queen)
    ...
    def get_chess_notation(self):
        """
        Return the move in algebraic chess notation (e.g., "e4", "Nf3", "Bxc6", etc.)
        """
        # Handle castling
        if self.is_castle_move:
            return "O-O" if self.end_col == 6 else "O-O-O"
            
        # Get the end square designation
        ...
        if self.piece_moved[1] == 'p':
            # Captures
            if self.is_capture:
                notation = self.cols_to_files[self.start_col] + "x" + end_square
            else:
                notation = end_square
        ...
        # For pieces (not pawns)
        piece_letter = self.piece_moved[1]
    ...
    def get_rank_file(self, row, col):
        """
        Convert a row and column into rank and file notation.
        """
